...
async def run_capture():
    ...
    if args.demo:
        ...
        def make_mosaic(images, pad=64, bg=(30, 30, 30)):
            ...
            max_x = max(x + w for (x, y), (w, h) in zip(positions, sizes, strict=False))
            max_y = max(y + h for (x, y), (w, h) in zip(positions, sizes, strict=False))
            mosaic = Image.new("RGBA", (max_x, max_y), bg)
            for (group, img), (x, y) in zip(images, positions, strict=False):
